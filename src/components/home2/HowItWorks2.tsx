...
export default function HowItWorks2() {
  return (
    <section className="py-24 relative overflow-hidden bg-[#0b1e3d] text-white">
      <div className="absolute inset-0 z-0 opacity-40">
        <div className="absolute inset-0 bg-gradient-to-br from-[#0b1e3d] via-gray-900 to-[#061226]" />
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 md:gap-2 relative">
          ...
          {howItWorksV2.map((step, i) => {
            ...
            return (
              ...
              >
                <div
                  className={`w-20 h-20 bg-[#0b1e3d] border-2 rounded-full flex items-center justify-center mb-6 ${
                    step.highlighted
                      ? "border-[#0058f7] shadow-[0_0_20px_rgba(0,88,247,0.2)]"
                      : "border-white/20"
                  }`}
                >
                  <Icon className={`h-8 w-8 ${step.highlighted ? "text-[#0058f7]" : "text-white"}`} />
                </div>
